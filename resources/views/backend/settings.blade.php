        ...
        <form action="{{ route('settings.update') }}" method="POST" class="col-6">
            ...
            <fieldset class="form-group settings">
                ...
                <label for="blue" class="paper-radio">
                    <input type="radio" name="main_template" id="blue"
                           value="blue" {{ ($settings->main_template==='blue')? "checked" : "" }} >
                    <span>Blue</span>
                </label>
                <label for="red" class="paper-radio">
                    <input type="radio" name="main_template" id="red"
                           value="red" {{ ($settings->main_template==='red')? "checked" : "" }} >
                    <span>Red</span>
                </label>
                <label for="purple" class="paper-radio">
                    <input type="radio" name="main_template" id="purple"
                           value="purple" {{ ($settings->main_template==='purple')? "checked" : "" }} >
                    <span>Purple</span>
                </label>
                <label for="sand" class="paper-radio">
                    <input type="radio" name="main_template" id="sand"
                           value="sand" {{ ($settings->main_template==='sand')? "checked" : "" }} >
                    <span>Sand</span>
                </label>
            </fieldset>
